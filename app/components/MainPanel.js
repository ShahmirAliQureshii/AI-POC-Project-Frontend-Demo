  ...
  return (
    <div className="rounded-2xl h-auto bg-white p-6 shadow-md">
      <div className="flex items-start justify-between gap-6">
        ...
        {/* Decorative video only on Home (aligns with aside because grid uses lg:col-span-3) */}
          ...
          {active === "Home" ? (
            <div className="hidden md:block w-full">
              <div className="relative overflow-hidden rounded-2xl shadow-2xl bg-black w-full h-64 sm:h-72 md:h-80 lg:h-96 transition-all">
                ...
                <div className="absolute -z-10 w-[400px] h-[400px] bg-blue-600/20 blur-[150px] rounded-full bottom-0 right-0 animate-pulse" />
              </div>
            </div>
          ) : (
            <div className="flex items-center gap-3">
              <div className="text-sm text-slate-500">Status</div>
              <div className="rounded-full bg-emerald-100 text-emerald-700 px-3 py-1 text-sm">
                Ready
              </div>
            </div>
          )}
      ...
      {/* Tab content: image / video / training / model / dashboard */}
      {active === "Image" && (
        <div className="mt-6 space-y-4">
          ...
          <input
            ...
            accept="image/*"
            multiple
            onChange={(e) => handleFilePlaceholder(e, "Images")}
            className="block w-full text-sm"
          />
          <div className="flex items-center gap-3 mt-4">
            <button
              onClick={() => processImages(document.getElementById("image-input")?.files)}
      ...
      {active === "Video" && (
        <div className="mt-6 space-y-4">
          ...
          <input
            ...
            type="file"
            accept="video/*"
            onChange={(e) => handleFilePlaceholder(e, "Video")}
            className="block w-full text-sm"
          />
      ...
      {/* Training / Model Viewer / Dashboard content preserved (keep as in your original) */}
      {active === "Training" && (
        <div className="mt-6 space-y-4">
          <label className="block text-sm font-medium text-slate-700">Upload Training Archive / Images</label>
          <input type="file" webkitdirectory="true" directory="true" multiple onChange={(e) => handleFilePlaceholder(e, "Training")} className="block w-full text-sm text-slate-600" />
          <div className="flex items-center gap-3 mt-4">
            ...
            <div className="p-4 rounded-xl bg-white shadow">
              ...
              <div className="mt-3 space-y-2">
                <div className="flex items-center justify-between">
                  <div>Red</div>
                  <div className="font-medium">{sampleStats.cards.red}</div>
                </div>
                <div className="flex items-center justify-between">
                  <div>Yellow</div>
                  <div className="font-medium">{sampleStats.cards.yellow}</div>
                </div>
                <div className="flex items-center justify-between">
                  <div>Green</div>
                  <div className="font-medium">{sampleStats.cards.green}</div>
                </div>
              </div>
            </div>
          </div>

          {/* Demo people cards (restored) */}
            ...
            {[
              {
                ...
                status: "Active",
                img:
                  "https://images.unsplash.com/photo-1545996124-1b1d9d0e6e8d?w=400&q=60&auto=format&fit=crop",
              },
              {
                name: "Priya Singh",
                status: "Idle",
              ...
              },
              {
                ...
                status: "Active",
                img:
                  "https://images.unsplash.com/photo-1549719389-5e63d49a2c8f?w=400&q=60&auto=format&fit=crop",
              },
            ].map((p) => (
              <div
                key={p.name}
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-2 text-sm text-slate-500">
                    Quick preview: 2 detections • best score 0.82
                  </div>
                </div>
              </div>
            ))}
